...
import type { ResultSet, Row, rawValue, intMode } from "./commons.js";
...
export function libsqlBatchStreamResParser(
    ...
    return res.step_results.map((r, i) => {
        ...
        else if (res.step_errors[i]) throw new ResponseError(res.step_errors[i]?.message!, res.step_errors[i]?.code);
        else return null;
    });
}
